...
def prepare_dataframe(dataset, split):
    """
    Prepares aligned DataFrames for the given evaluation split.

    Returns 6 values:
      X_train, X_test   – student answer text Series
      y_train, y_test   – integer label Series
      train_ref, test_ref – reference answer Series (needed for semantic
                            feature extraction: Jaccard / token-density)
    """
