...
function GradePanel() {
  ...
  const fileInputRef = useState(null); // Used to trigger hidden file input
  ...
  const handleOCRUpload = async (e) => {
    ...
    try {
      const { data } = await axios.post('http://localhost:3000/api/ocr', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      if (data.transcribed_text) {
        setStudent(data.transcribed_text);
      } else {
        setError('OCR returned empty text. Please try a clearer image.');
      }
    } catch (err) {
      console.error(err);
        ...
        <div className="space-y-5">
          ...
          <div>
            <div className="flex justify-between items-end mb-2">
              <label className="text-sm font-medium text-slate-400">Student Answer</label>
              <div className="relative">
                ...
                <label 
                  htmlFor="ocr-upload"
                  className={cn(
                    "text-xs font-bold py-1.5 px-3 rounded-full cursor-pointer transition-all flex items-center gap-1.5",
                    ocrLoading 
                      ? "bg-slate-700 text-slate-500 cursor-not-allowed" 
                      : "bg-blue-900/40 text-blue-400 hover:bg-blue-800/60 border border-blue-500/30"
                  )}
                >
                  {ocrLoading ? <Loader2 size={12} className="animate-spin" /> : <BarChart3 size={12} />}
                  {ocrLoading ? 'Scanning…' : 'Scan Handwriting'}
                </label>
              </div>
            </div>
            <textarea 
              ...
              placeholder="Paste student response or use OCR button above..."
            />
          </div>
